      ...
      <header style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => navigate('/admin')} style={{
            ...
            border: 'none', padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600,
          }}>
            Price Board
          </button>
          <button onClick={() => { localStorage.removeItem('pb_token'); navigate('/admin/login'); }} style={{
            background: 'rgba(255,255,255,0.15)', color: '#fff',
            border: 'none', padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600,
          }}>
            Logout
        ...
        {loading ? (
          ...
          <>
            <div style={{
              ...
              <p style={{ fontSize: 36, fontWeight: 700 }}>{formatPrice(data?.grandTotal || 0)}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 16 }}>
              {data?.summary?.map(w => (
                <div key={w.worker} style={{
                  ...
                  <p style={{ fontSize: 22, fontWeight: 700, color: COLORS[w.worker] || '#1a6b3c' }}>
                    {formatPrice(w.total)}
                  </p>
                ...
                {history.map(e => (
                  <div key={e.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: COLORS[e.worker] || '#1a6b3c' }} />
                      <div>
